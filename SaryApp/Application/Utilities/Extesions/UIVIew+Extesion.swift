...
extension UITableView{
    ...
    func registerHeader<T: UITableViewHeaderFooterView>(headerType: T.Type, bundle: Bundle? = nil){
        let className = String(describing: headerType.self)
        let nib = UINib(nibName: className, bundle: bundle)
    ...
    func dequeueReusableHeader<T:UITableViewHeaderFooterView>(with type: T.Type, section:Int)->T{
        return self.dequeueReusableHeaderFooterView(withIdentifier: String(describing: type.self)) as! T
    }
}
...
extension Refreshable {
    ...
    func Loading(){
        print("Loading")
        showIndicator(withTitle: "Loding...", and: "")
    }
...
extension Dictionary {
    var json: String {
        do {
            ...
            return String(bytes: jsonData, encoding: String.Encoding.utf8) ?? "invalid JSON"
        } catch {
            return "invalid JSON"
        }
    }
